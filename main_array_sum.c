...
int main()
{
    size_t const array_sz = 100000;
    size_t const array_mem_sz = array_sz * sizeof(cl_uint);
    char const* const kernel_file_name = "array_sum.cl";
    char const* const kernel_name = "array_sum";
    size_t const file_load_sz = 1024 * 16;

    cl_uint* const a = (cl_uint*) malloc(array_mem_sz);
    cl_uint* const b = (cl_uint*) malloc(array_mem_sz);
    cl_uint* const c = (cl_uint*) malloc(array_mem_sz);
    ...
    cl_mem mem1 = clCreateBuffer(context, CL_MEM_READ_ONLY, array_mem_sz, 0, &error_code);
    CHECK_ERR("Error creating buffer", error_code, exit3);
    cl_mem mem2 = clCreateBuffer(context, CL_MEM_READ_ONLY, array_mem_sz, 0, &error_code);
    CHECK_ERR("Error creating buffer", error_code, exit3);
    cl_mem mem3 = clCreateBuffer(context, CL_MEM_READ_WRITE, array_mem_sz, 0, &error_code);
    CHECK_ERR("Error creating buffer", error_code, exit3);
    ...
    size_t work_size = array_sz;
    cl_event run_event;
    clEnqueueNDRangeKernel(queue, kernel, 1, &work_offset, &work_size, 0, 0, 0, &run_event);
    clEnqueueReadBuffer(queue, mem3, true, 0, array_mem_sz, c, 0, 0, 0);
